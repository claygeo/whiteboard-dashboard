...
const DataTable = ({ tableData, onRowDoubleClick, renderPagination, onDelete }) => {
  ...
  const searchedData = data.filter(item => {
    ...
    return (
      ...
      (item.line_status?.toLowerCase() || '').includes(searchText)
    );
  });
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    const aValue =
      sortConfig.key === 'start_time' || sortConfig.key === 'end_time'
        ...
        ? a[sortConfig.key]
        : sortConfig.key === 'line_status'
        ? a[sortConfig.key] || 'Open'
        : a[sortConfig.key];
    const bValue =
      sortConfig.key === 'start_time' || sortConfig.key === 'end_time'
      ...
      <table className="batches-table">
            ...
            <th>
              ...
              <div className="filter-sort">
                <input
                  type="text"
                  placeholder="Filter..."
                  value={filterConfig.line_status || ''}
                  onChange={(e) => handleFilterChange('line_status', e.target.value)}
                />
                <button onClick={() => handleSort('line_status')}>
                  {sortConfig.key === 'line_status' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}
                </button>
              </div>
            </th>
            <th>Actions</th>
        ...
        <tbody>
          {sortedData.map(row => (
            <tr
              key={row.id}
              onDoubleClick={() => !row.is_locked && onRowDoubleClick(row)}
              className={row.is_locked ? 'locked-row' : ''}
            >
              <td>{row.ubi_no}</td>
